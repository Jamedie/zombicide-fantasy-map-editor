...
let CATALOG_POLICY = { doorPlacement: 'anchor-required', freeCoordinates: false };
const PRODUCTS = {
...
const CATALOG = TILE_PRODUCT_RANGES.flatMap(range => Array.from({ length: range.to - range.from + 1 }, (_, offset) => range.from + offset).flatMap(number => ['R', 'V'].map(face => ({ number, face, product: range.product })))).map((entry, index) => ({
  ...
  image: path.resolve(CLI_DIR, `../assets/tiles/${entry.number}${entry.face}.webp`),
...
function validateMission(mission, collection) {
  ...
  const instanceIds = new Set(); const cells = new Set();
  ...
  for (const [index, marker] of mission.markers.entries()) {
    ...
    if (marker.anchor) {
      ...
      const anchor = anchors.find(entry => entry.id === marker.anchor);
      if (!anchor) errors.push({ code: 'MARKER_ANCHOR', path: where, message: `slot « ${marker.anchor} » introuvable.` });
      else { const expected = rotatePoint(anchor.x, anchor.y, tile.rotation || 0); if (Math.abs(marker.x - expected.x) > .002 || Math.abs(marker.y - expected.y) > .002) errors.push({ code: 'SLOT_POSITION', path: where, message: `les coordonnées du marqueur ne correspondent pas au slot « ${marker.anchor} » après rotation.` }); }
    }
  }
...
function renderPng(svg, output, width, height) {
  ...
  const result = spawnSync(edge, ['--headless=new', '--disable-gpu', '--hide-scrollbars', `--window-size=${width},${height}`, `--screenshot=${path.resolve(output)}`, pathToFileURL(svgFile).href], { encoding: 'utf8', timeout: 30000 });
...
if (command === 'catalog') {
  ...
  const context = { tool: 'zombicide-map', version: VERSION, constraints: { grid: { columns: 'integer 1..4', rows: 'integer 1..4' }, rotations: [0, 90, 180, 270], normalizedMarkerCoordinates: true, slotPolicy: CATALOG_POLICY, markers: Object.keys(MARKERS), availableTiles: available }, missionContract: { format: 'zombicide-map', version: 1, name: 'string', grid: { columns: 3, rows: 2 }, tiles: [{ instanceId: 'unique string', catalogId: 'available tile id', code: 'catalog code', face: 'R|V', column: 0, row: 0, rotation: 0, customDoorAnchors: [] }], markers: [{ id: 'unique string', type: 'marker type; doors require anchor from selected tile', tile: 'tile instanceId', anchor: 'required catalog slot id for doors', x: 'derived from slot and rotation', y: 'derived from slot and rotation', label: 'string' }], render: { showTileNames: true, showLegend: true, background: '#24282d' } } };
