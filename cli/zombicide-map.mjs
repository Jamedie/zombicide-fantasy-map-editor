...
import { CATALOG_PREFERRED_MARKERS, CELL_CENTER_MARKERS, DOOR_MARKERS, EDGE_ANCHOR_MARKERS, INVASION_MARKERS, TOKEN_MARKERS } from '../app/scripts/token.js';
...
function markerImageDataUri(type, open = false) {
  const marker = MARKER_DATA[type];
  const image = open && marker?.imageOpen ? marker.imageOpen : marker?.image;
...
function svgMarkerContent(type, id, x, y, size, radius, label, fontSize = 12, open = false) {
  const image = markerImageDataUri(type, open);
  if (image) {
    const marker = MARKER_DATA[type];
...
function validateMission(mission, collection) {
  ...
  for (const [index, marker] of mission.markers.entries()) {
    ...
    if (marker.open !== undefined && (!DOOR_MARKERS.has(marker.type) || typeof marker.open !== 'boolean')) errors.push({ code: 'DOOR_STATE', path: where, message: 'open est réservé aux portes et doit être un booléen.' });
    ...
    if (DOOR_MARKERS.has(marker.type) && CATALOG_POLICY.doorPlacement === 'anchor-required' && !marker.anchor) errors.push({ code: 'DOOR_ANCHOR_REQUIRED', path: where, message: 'une porte doit référencer un slot autorisé du catalogue.' });
    if (marker.anchor) {
      ...
      const anchors = DOOR_MARKERS.has(marker.type) ? [...(data?.doorAnchors || []), ...(tile?.customDoorAnchors || [])] : (data?.slots || []).filter(slot => slot.type === marker.type);
      ...
      if (!anchor) errors.push({ code: 'MARKER_ANCHOR', path: where, message: `slot « ${marker.anchor} » introuvable.` });
        ...
        if (!DOOR_MARKERS.has(marker.type)) {
  ...
  for (const [index, tile] of mission.tiles.entries()) {
    ...
    for (const slot of data?.doorAnchors?.filter(anchor => anchor.requiresDoor) || []) {
      if (!requiredDoorSlotIsActive(mission, tile, slot)) continue;
...
function requiredDoorSlotIsActive(mission, tile, slot) {
  const point = rotatePoint(slot.x, slot.y, tile.rotation || 0);
  const connection = doorConnection(tile, point, slot.id);
  if (connection.axis === 'internal') return true;
  return mission.tiles.some(otherTile => {
    if (otherTile === tile) return false;
    const data = CATALOG.find(entry => entry.id === otherTile.catalogId);
    return (data?.doorAnchors || []).some(otherSlot => {
      const otherPoint = rotatePoint(otherSlot.x, otherSlot.y, otherTile.rotation || 0);
      return sameDoorConnection(connection, doorConnection(otherTile, otherPoint, otherSlot.id));
    });
  });
}
function markerRenderPoint(mission, marker) {
  ...
  if (DOOR_MARKERS.has(marker.type) && marker.anchor) {
    const data = CATALOG.find(entry => entry.id === tile.catalogId);
...
function missionSvg(mission, validation) {
  ...
  const markers = mission.markers.map(marker => { const point = displayPoints.get(marker.id) || markerRenderPoint(mission, marker); if (!point) return ''; const x = point.x * TILE_SIZE; const y = point.y * TILE_SIZE; const square = DOOR_MARKERS.has(marker.type) || ['invasion', 'exit', 'gate', 'crypt', 'crypt-yellow'].includes(marker.type); const size = DOOR_MARKERS.has(marker.type) ? 36 : 35; return `<g>${svgMarkerContent(marker.type, marker.id, x, y, size, square ? 3 : size / 2, marker.label, 12, marker.open === true)}</g>`; }).join('');
